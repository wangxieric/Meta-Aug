...
def train(model, dataloader, optimizer, epoch_number, max_gradient_norm):
    ...
    for batch_index, (batch_seqs_feat, batch_seq_masks, batch_labels, batch_seqs_atr) in enumerate(tqdm_batch_iterator):
...
def validate(model, dataloader):
    ...
    with torch.no_grad():
        for (batch_seqs_feat, batch_seq_masks, batch_labels, batch_seqs_atr) in dataloader:
            # Move input and output data to the GPU if one is used.
            seqs = batch_seqs_feat.to(device)
            masks = batch_seq_masks.to(device)
            labels = batch_labels.to(device)
            seqs_atr = batch_seqs_atr.to(device) if batch_seqs_atr is not None else batch_seqs_atr
            loss, logits, probabilities = model(seqs, masks, labels, seqs_atr)
            running_loss += loss.item()
...
def test(model, dataloader):
    ...
    # Deactivate autograd for evaluation.
    with torch.no_grad():
        for (batch_seqs, batch_seq_masks, batch_labels, batch_seqs_atr) in dataloader:
            batch_start = time.time()
            # Move input and output data to the GPU if one is used.
            seqs, masks, labels = batch_seqs.to(device), batch_seq_masks.to(device), batch_labels.to(device)
            seqs_atr = batch_seqs_atr.to(device) if batch_seqs_atr is not None else batch_seqs_atr
            _, _, probabilities = model(seqs, masks, labels, seqs_atr)
